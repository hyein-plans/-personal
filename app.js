...
function archive(){const meta={kmr:{name:'한국경영인증원 통합플랫폼',desc:'분리된 업무를 웹 기반 통합플랫폼으로 재설계',tags:['업무시스템','통합플랫폼','레거시 전환'],icon:'document'},cj:{name:'CJ프레시웨이 회원 전용 폐쇄몰',desc:'사용자 주문부터 운영자의 배송·반품까지 연결',tags:['커머스','B2B','다중배송'],icon:'structure'},daesung:{name:'대성 CRM',desc:'대리점 주문·출고·AS 업무를 연결한 CRM',tags:['CRM','B2B','권한'],icon:'data'},'us-crm':{name:'대성 미주 CRM',desc:'제조번호 중심 고객·제품·서비스 데이터 허브',tags:['CRM','Data Flow','Serial Number'],icon:'aggregate'},'kr-crm':{name:'대성 국내 CRM',desc:'상담·배정·이관·권한을 연결한 국내 CRM',tags:['CRM','AS','Workflow'],icon:'role'},o2o:{name:'O2O 픽업 주문 서비스',desc:'온라인 주문과 매장 운영을 연결한 픽업 서비스',tags:['O2O','Order','Store'],icon:'company'},campaign:{name:'캠페인 분석 CRM',desc:'분산 데이터를 통합·가공해 당일 확인 가능한 흐름 구축',tags:['CRM','Data','Dashboard'],icon:'data'},other:{name:'Other Projects',desc:'대표 프로젝트 밖 다양한 도메인 경험',tags:['Policy','Workflow','IA'],icon:'manual'}};const cards=[...projects,other].map((p,i)=>{const m=meta[p.id]||p;return `<a class="project-card" href="#project/${p.id}"><span class="project-no">${String(i+1).padStart(2,'0')}</span><span class="archive-icon" aria-hidden="true">${kmrIcon(m.icon)}</span><div class="archive-copy"><h3>${m.name}</h3><p>${m.desc||m.one}</p>${chips(m.tags)}</div><span class="arrow" aria-hidden="true">→</span></a>`}).join('');return `<section class="section archive" id="projects"><div class="section-head"><div><span class="section-kicker">Selected Work</span><h2>PROJECT ARCHIVE</h2></div><div class="archive-side"><p>다양한 산업과 비즈니스 환경에서<br>사용자와 운영을 연결하는 서비스를 기획했습니다.</p><a class="archive-all" href="#project/other">전체 프로젝트 보기　→</a></div></div><div class="project-grid">${cards}</div></section>`}
function skills(){
  ...
  return `<section class="home-bottom home-after" id="skills"><section class="home-domain"><div class="home-domain-title"><span class="section-kicker">EXPERTISE</span><h2>다양한 도메인에서 쌓아온<br>서비스 기획 경험</h2></div><p class="home-domain-copy">비즈니스의 특성과 사용자의 맥락을 이해하고,<br>문제를 구조화하여 실행 가능한 서비스로 만듭니다.</p><div class="home-domain-grid">${domains.map(x=>`<article><span class="home-mini-icon">${kmrIcon(x[0])}</span><div><b>${x[1]}</b><small>${x[2]}</small></div></article>`).join('')}</div></section><aside class="home-tools"><h2>SKILLS & TOOLS</h2><p>기획과 실행을 위한 도구들로<br>아이디어를 현실에 옮깁니다.</p><div class="home-tool-list">${tools.map(x=>`<article><span class="home-tool-icon">${kmrIcon(x[0])}</span><div><b>${x[1]}</b><small>${x[2]}</small></div></article>`).join('')}</div><div class="home-certificate"><b>자격증</b><span>DAsP 데이터아키텍처 준전문가　|　2023.05</span></div></aside><section class="home-work"><div><span class="section-kicker">MY WORK STYLE</span><h3>좋은 서비스는, 차근차근 만들어갑니다.</h3></div><div class="home-work-list">${work.map((x,i)=>`<article><span class="home-work-icon">${kmrIcon(x[0])}</span><div><small>0${i+1}</small><b>${x[1]}</b><p>${x[2]}</p></div></article>`).join('')}</div></section></section>`}
function contact(){return `<section class="contact-band" id="contact"><div class="contact-face"></div><h2>더 나은 서비스를 만드는 여정에<br>함께하고 싶습니다.</h2><a class="contact-button" href="mailto:gpdls6875@gmail.com">Contact Me　→</a><p class="contact-extra">사람과 비즈니스를 연결하는<br><b>실행력 있는 서비스 기획</b></p></section>`}
...
function kmrDetail(print=false){
  ...
  return `<div class="page detail kmr-detail ${print?'detail-page':''}">
    ...
    <section class="kmr-hero">
      ...
      <aside class="kmr-challenge"><small>PROJECT CHALLENGE</small><ul><li>프로젝트 중간 인계</li><li>요구사항·업무 범위 미정리</li><li>PowerBuilder → Web 전환</li></ul><p>진행 중인 프로젝트를 이어받아 실제 업무 기준부터 다시 정리했습니다.</p></aside>
    </section>
    <div class="kmr-two-col">
      <section class="kmr-section kmr-context">${cap('01','Background','왜 필요했을까요?')}<p>일부 화면은 있었지만 요구사항·업무 흐름·개발 범위가 정리되지 않은 상태에서 프로젝트를 인계받았습니다. 레거시 업무시스템을 Web으로 전환하며 실제 처리 기준부터 다시 정리했습니다.</p>${check(['프로젝트 중간 인계','요구사항·업무 흐름·개발 범위 미정리','PowerBuilder 기반 레거시 업무시스템의 Web 전환'])}</section>
      <section class="kmr-section kmr-context">${cap('02','Objective','무엇을 목표로 했나요?')}<p>기존 화면을 옮기는 데 그치지 않고, 실제 업무 목적을 기준으로 화면과 프로세스를 재구성해 후속 업무까지 연결되는 통합 구조를 설계했습니다.</p>${check(['요구사항·업무 흐름 재정리','화면 구조 재설계','데이터·후속 업무 연결 기준 정의','개발 협의·QA·운영 인계'])}</section>
    </div>
    ...
    <section class="kmr-section kmr-process kmr-process-enhanced">${cap('04','Process','분리된 두 업무를 하나의 흐름으로 연결했습니다.')}<p class="kmr-side-copy">실적 등록부터 전표 발행까지,<br>하나의 업무 흐름에서 처리하도록 재설계했습니다.</p><div class="kmr-process-grid"><article class="kmr-process-panel"><div class="kmr-panel-head"><span>분리된 업무 흐름</span><b>As-Is</b></div>${flow(['실적 등록','등록 완료','다른 메뉴로 이동','별도 전표 발행'])}<div class="kmr-process-result is-before"><small>문제점</small><strong>업무가 서로 분리되어 단계가 끊어지고 있었습니다.</strong></div></article><span class="kmr-process-arrow"><small>분리</small>통합</span><article class="is-tobe kmr-process-panel"><div class="kmr-panel-head"><span>하나로 연결된 업무 흐름</span><b>To-Be</b></div>${flow(['실적 추가·삭제','전표 전송','결과 확인'])}<div class="kmr-process-result is-after"><small>개선 효과</small><strong>같은 업무 흐름 안에서 처리하고 결과까지 바로 확인</strong></div></article></div></section>
    ...
    <section class="kmr-section kmr-outcome">${cap('06','Outcome','어떤 결과를 만들었나요?')}<div class="kmr-outcome-grid"><article class="kmr-highlight"><small>핵심 결과</small><h3>분리되어 있던 실적 등록·전표 발행 절차를 <em>하나의 업무 흐름</em>으로 통합했습니다.</h3><p>업무 목적을 기준으로 화면과 프로세스를 재구성하고, 앞 단계의 정보가 후속 업무까지 이어지도록 설계했습니다.</p></article><div class="kmr-result-list">${results.map(x=>`<article><span>✓</span><div><h3>${x[0]}</h3><p>${x[1]}</p></div></article>`).join('')}</div></div></section>
...
function cjDetail(print=false){
  ...
  return `<div class="page detail cj-detail ${print?'detail-page':''}">
    ...
    <section class="cj-section cj-solution">${cap('04','Solution','기존 기능과 추가 개발 범위를 구분했습니다.')}<p class="cj-side-copy">퍼스트몰의 구조를 분석해<br>고객사 환경에 맞는 구현 범위를 정의했습니다.</p><div class="cj-solution-grid">${solutions.map(x=>`<article><span class="cj-card-icon">${kmrIcon(x[0])}</span><h3>${x[1]}</h3><p>${x[2]}</p>${x[3]?`<small>${x[3]}</small>`:''}</article>`).join('')}</div><p class="cj-takeaway">퍼스트몰의 기존 기능과 필요한 추가 기능을 구분해 구현 범위를 정의했습니다.</p></section>
    <section class="cj-section cj-order cj-order-enhanced">${cap('05','Order Structure','대량·다중배송 주문 구조를 어떻게 설계했나요?')}<p class="cj-side-copy">하나의 주문 안에서도 여러 배송지를 설정하고, 배송지별로 독립적인 출고·반품 처리가 가능하도록 주문 구조를 설계했습니다.</p><div class="cj-order-visual"><article class="cj-order-total"><small>주문 전체</small><h3>#2023-0001</h3><p>5명의 고객사</p><p>CJ 통합회사</p><p>IT 용품 외 2건</p><p>주문일 2023.07.31</p><p>주문 금액 1,250,000원</p><p>상품 수 20 · 5박</p><p>배송지 수 3개</p></article><div class="cj-order-branch"><i>→</i><div class="cj-addresses"><div class="cj-address cj-address-head"><b>배송지</b><span>상품</span><span>수량</span><span>배송비</span><em>출고 상태</em></div>${[['배송지 A','서울시 강남구 테헤란로 123','상품 1, 2','2','3,000원','출고완료','is-complete'],['배송지 B','서울시 강남구 코엑스대로 234','상품 3','1','0원','출고대기','is-pending'],['배송지 C','서울시 강남구 영동대로 567','상품 4, 5','2','5,000원','부분출고','is-partial']].map(x=>`<div class="cj-address"><b>${x[0]}<small>${x[1]}</small></b><span>${x[2]}</span><span>${x[3]}</span><span>${x[4]}</span><em class="${x[6]}">${x[5]}</em></div>`).join('')}</div></div><aside class="cj-order-note"><span>ⓘ</span><p>배송지별로 출고 상태가 다를 수 있으며, 반품 또한 배송지 단위로 처리됩니다.</p></aside></div></section>
    <section class="cj-section cj-return cj-return-enhanced">${cap('06','Return','반품 흐름은 어떻게 설계했나요?')}<p class="cj-side-copy">배송지별 출고 단위와 반품 대상 품목을<br>같은 기준으로 연결했습니다.</p><div class="cj-return-grid"><div class="cj-return-flows"><div><small>다중배송 주문</small>${flow(['배송지 선택','해당 배송지 상품 확인','반품 사유 선택','반품 신청','처리 상태 확인'])}</div><div><small>단일배송 주문</small>${flow(['배송지 선택 생략','반품 상품 선택','반품 사유 선택','반품 신청','처리 상태 확인'])}</div></div></div><div class="cj-return-notes"><p><span>✓</span>단일은 배송지 단위로 신청하여, 해당 배송지의 상품·수량만 처리됩니다.</p><p><span>✓</span>다중배송 주문은 배송지 선택 단계부터 배송 단위로 반품할 수 있습니다.</p></div></section>
...
function daesungDetail(print=false){
  ...
  const roles=[
    ['role','사용자·권한 구조 설계','대리점·영업사원·본사의 조회·처리 범위를 역할 기준으로 정의했습니다.'],
    ['handoff','주문·출고 프로세스 설계','대리점 직접 주문과 영업사원 대리주문, 주문 이후 출고 확인 흐름을 설계했습니다.'],
    ['screen','App / Web 역할 구분','현장 업무와 관리 업무의 목적에 맞춰 사용 화면과 운영 화면을 구분했습니다.'],
  ...
  return `<div class="page detail ds-detail ${print?'detail-page':''}">
    ...
    <section class="ds-hero">
      <div class="ds-hero-main"><div class="ds-overline"><b>03</b>${chips(['CRM','B2B','Permission','Order / Shipping','App / Web'])}</div><span class="section-kicker">PROJECT CASE STUDY</span><h1>2021 · 대성 CRM</h1><p>대리점·영업사원·본사의 주문과 출고 업무를 CRM 안에서 직접 처리할 수 있도록,<br>사용자별 권한과 시스템 간 처리 기준을 설계했습니다.</p></div>
      <aside class="ds-project-structure"><div class="ds-structure-head"><b>Project Structure</b><small>2021.03 – 2023.02</small></div><div class="ds-structure-row"><span>${kmrIcon('role')}</span><b>USERS</b><p>대리점 · 영업사원 · 본사</p></div><div class="ds-structure-row"><span>${kmrIcon('structure')}</span><b>SYSTEMS</b><p>CRM · EPICO · WMS</p></div><p class="ds-structure-summary">사용자별 처리 범위와 시스템 간 업무 연결 기준을 함께 설계했습니다.</p></aside>
    </section>
    <div class="ds-two-col">
      <section class="ds-section ds-context">${cap('01','Background','왜 필요했을까요?')}<p>대리점·영업사원·본사처럼 사용자 유형이 다양했고, CRM에서 시작한 주문은 EPICO와 WMS의 후속 처리까지 이어지는 구조였습니다.</p>${checks(['사용자 유형별 업무 범위 상이','CRM · EPICO · WMS 간 업무 연결 필요','주문·출고 상태에 따라 후속 처리 방식 상이'])}</section>
      <section class="ds-section ds-context">${cap('02','Objective','무엇을 목표로 했나요?')}<p>사용자별 권한과 주문·출고 처리 기준을 명확히 하고, 시스템 간 업무가 끊기지 않도록 구조화하는 것을 목표로 했습니다.</p>${checks(['사용자 유형별 조회·처리 권한 정의','대리점 직접 주문 / 영업사원 대리 주문 구조','주문·출고·취소·반품 상태 기준 정의','App / Web 역할 구분','CRM · EPICO · WMS 업무 연결'])}</section>
    </div>
    ...
    <section class="ds-section ds-users ds-users-enhanced">${cap('04','User / Permission','사용자별로 조회·처리 가능한 업무 범위를 구분했습니다.')}<p class="ds-side-copy">사용자별 업무 범위와 권한을 한눈에 정리했습니다.</p><div class="ds-permission-layout"><div><div class="ds-permission-table"><div class="ds-permission-row ds-permission-head"><b>업무</b><b>대리점</b><b>영업사원</b><b>본사</b></div>${[['직접 주문','●','—','—'],['대리 주문','—','●','—'],['주문·출고 조회','●','●','●'],['취소·반품 요청','●','●','—'],['전체 주문 상태 확인','—','○','●'],['출하 처리','—','—','●'],['통계 조회','—','—','●']].map(x=>`<div class="ds-permission-row"><b>${x[0]}</b><span>${x[1]}</span><span>${x[2]}</span><span>${x[3]}</span></div>`).join('')}</div><p class="ds-permission-legend"><span>● 가능</span><span>○ 제한적 가능</span><span>— 불가</span></p></div><aside class="ds-user-summary"><h3>사용자별 주요 역할</h3>${[['project','대리점','직접 주문·조회·취소/반품 요청'],['role','영업사원','담당 대리점 대리 주문·지원'],['company','본사','전체 주문 상태 확인·후속 처리']].map(x=>`<article><span>${kmrIcon(x[0])}</span><div><b>${x[1]}</b><p>${x[2]}</p></div></article>`).join('')}</aside></div><p class="ds-permission-takeaway">대리점이 직접 처리할 수 있는 범위를 확대하면서도, 영업사원과 본사의 역할은 권한 기준으로 분리했습니다.</p></section>
    <section class="ds-section ds-system ds-system-enhanced">${cap('05','System Flow','CRM에서 시작한 주문이 시스템 간 업무 흐름으로 이어지도록 설계했습니다.')}<p class="ds-side-copy">주문부터 출고, 최종 확인까지 시스템별 역할과 전달 기준을 구분했습니다.</p><div class="ds-system-panel"><div class="ds-system-flow">${systems.map((x,i)=>`${i?`<span class="ds-arrow"><small>${i===1?'주문 전달':i===2?'출하 처리':'결과 반영'}</small>→</span>`:''}<article><strong>${x[0]}</strong><b>${x[1]}</b><p>${x[2]}</p></article>`).join('')}</div></div></section>
    <section class="ds-section ds-status">${cap('06','Status / Follow-up','상태에 따라 가능한 후속 처리를 구분했습니다.')}<p class="ds-side-copy">현재 상태에 따라 사용자가 할 수 있는 다음 행동이 달라지도록 설계했습니다.</p><div class="ds-status-grid">${statuses.map(x=>`<article><span class="ds-card-icon">${kmrIcon(x[0])}</span><h3>${x[1]}</h3><p>${x[2]}</p></article>`).join('')}</div></section>
    <section class="ds-section ds-appweb">${cap('07','App / Web','현장과 운영 업무 특성에 맞게 기능을 구분했습니다.')}<div class="ds-appweb-grid"><article><span class="ds-card-icon">${kmrIcon('role')}</span><div><h3>App — 현장 업무</h3><p>주문·조회·처리 중심</p></div></article><article><span class="ds-card-icon">${kmrIcon('screen')}</span><div><h3>Web — 관리 업무</h3><p>관리·조회·운영 중심</p></div></article></div></section>
    <section class="ds-section ds-outcome">${cap('08','Outcome','어떤 결과를 만들었나요?')}<div class="ds-outcome-grid"><article class="ds-highlight"><small>핵심 결과</small><h3>대리점이 <em>직접 처리할 수 있는</em> 주문·조회·취소·변경·반품 업무 범위를 확대했습니다.</h3><p>사용자별 권한과 상태 기준을 정의하고, CRM·EPICO·WMS 간 업무 연결 구조를 설계했습니다.</p></article><div class="ds-result-list">${results.map(x=>`<article><span>✓</span><div><h3>${x[0]}</h3><p>${x[1]}</p></div></article>`).join('')}</div></div></section>
...
function usCrmDetail(print=false){
  ...
  const roles=[
    ...
    ['data','제조번호 기반 데이터 구조 설계','Serial Number를 기준으로 제품·설치 정보와 서비스 기록이 이어지도록 구성했습니다.'],
  ...
  const hubIcons={customer:kmrIcon('role'),product:'<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="m4 8 8-4 8 4v9l-8 4-8-4z"/><path d="m4 8 8 4 8-4M12 12v9"/></svg>',installation:kmrIcon('tools'),shipping:'<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h11v11H3zM14 10h3l3 3v4h-6z"/><circle cx="7" cy="18" r="2"/><circle cx="17" cy="18" r="2"/></svg>',service:'<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M5 13v-1a7 7 0 0 1 14 0v1"/><path d="M5 13h2v5H5zm12 0h2v5h-2zM7 20h7"/></svg>',return:'<svg viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"><path d="M6 8h11v11H6z"/><path d="m10 5-3 3 3 3M7 8h10"/></svg>'};
  const reuse=[
    ['data','제조번호 기반 제품·설치 정보 재사용','Serial Number 입력','Verify','제품 / 설치 정보 조회','자동 입력','필요 시 수정'],
    ['role','기존 고객 정보 재사용','고객 검색','기존 고객 선택','고객 정보 자동 반영','필요 시 수정','서비스 등록']
  ];
  const platforms=[
    ['role','App — 현장 실행 중심','현장에서 서비스와 반품 업무를 처리합니다.',['서비스 업무','반품 신청','현장 기록·조회']],
    ['screen','Web — 관리·승인 중심','관리·승인 업무와 운영 이력을 관리합니다.',['회원/서비스 관리','반품 승인','교육 설정','업무 기록 관리']]
  ];
  const results=[
  ...
  return `<div class="page detail us-detail ${print?'detail-page':''}">
    ...
    <section class="us-hero">
      <div class="us-hero-main"><div class="us-overline"><b>04</b>${chips(['CRM','Data Flow','Serial Number','App / Web','Service'])}</div><span class="section-kicker">PROJECT CASE STUDY</span><h1>2025 · 대성 미주 CRM</h1><p>고객·제품·설치·서비스 이력을 제조번호 기준으로 연결하고, 기존 정보를 서비스 업무에 재사용할 수 있도록 구성했으며,<br>현장 App과 관리자 Web의 역할을 구분해 서비스·반품 업무를 시스템화했습니다.</p></div>
      <aside class="us-project-structure"><div class="us-structure-head"><b>Project Structure</b><small>2025.05 ~ 2026.06</small></div><div class="us-project-meta">${[['company','회사','가비아CNS'],['role','역할','서비스기획'],['project','프로젝트','대성 미주 CRM 신규 구축']].map(x=>`<p><span>${kmrIcon(x[0])}</span><b>${x[1]}</b><em>${x[2]}</em></p>`).join('')}</div><div class="us-core-key"><small>CORE KEY</small><b>${kmrIcon('data')} Serial Number <em>/ 제조번호</em></b></div><div class="us-connected-data"><small>CONNECTED DATA</small><p>${['Customer','Product','Installation','Service','Return'].map(x=>`<span>${x}</span>`).join('')}</p></div><p class="us-structure-summary">제조번호를 공통 기준으로 고객·제품·서비스 이력을 연결했습니다.</p></aside>
    </section>
    <div class="us-two-col">
      <section class="us-section us-context">${cap('01','Background','왜 필요했을까요?')}<p>고객·제품·서비스 정보가 담당자별 파일과 업무 방식에 따라 분산되어, 기존 정보를 반복 입력하고 이력을 이어서 확인하기 어려웠습니다.</p>${checks(['고객·제품·서비스 정보가 담당자별 방식으로 분산','기존 정보 반복 입력','이전 서비스 이력 확인 및 연결 어려움','현장 사용자와 관리자 업무 차이'])}</section>
      <section class="us-section us-context">${cap('02','Objective','무엇을 목표로 했나요?')}<p>제조번호를 중심으로 고객·제품·설치·서비스 정보를 연결하고, 기존 데이터를 재사용하면서 App과 Web에서 역할에 맞게 처리할 수 있는 CRM 구조를 설계했습니다.</p>${checks(['제조번호 중심 데이터 연결','고객·제품·설치·서비스 이력 연결','기존 데이터 조회·재사용','App / Web 역할 분리','권한·상태·승인 처리 기준 정의'])}</section>
    </div>
    ...
    <section class="us-section us-hub us-hub-enhanced">${cap('04','Data Hub','제조번호를 중심으로 고객·제품·서비스 정보를 연결했습니다.')}<p class="us-side-copy">하나의 제조번호로 고객부터 제품, 설치, 출고, 서비스, 반품까지 모든 이력을 연결합니다.</p><div class="us-hub-canvas"><svg class="us-hub-connectors" viewBox="0 0 1000 300" preserveAspectRatio="none" aria-hidden="true"><path d="M348 58 424 108M348 150h76M348 242l76-50M576 108l76-50M576 150h76M576 192l76 50"/></svg><div class="us-hub-side us-hub-left">${hubLeft.map((x,i)=>`<article class="hub-${i}"><div><b>${x[0]}</b><p>${x[1]}</p></div><span>${hubIcons[['customer','product','installation'][i]]}</span></article>`).join('')}</div><div class="us-hub-core"><span>${kmrIcon('data')}</span><b>Serial<br>Number</b><small>제조번호</small></div><div class="us-hub-side us-hub-right">${hubRight.map((x,i)=>`<article class="hub-right-${i}"><span>${hubIcons[['shipping','service','return'][i]]}</span><div><b>${x[0]}</b><p>${x[1]}</p></div></article>`).join('')}</div></div><p class="us-hub-takeaway">각 시스템과 업무에서 같은 제조번호를 기준으로 이력을 이어서 확인할 수 있도록 설계했습니다.</p></section>
    <section class="us-section us-reuse-enhanced">${cap('05','Data Reuse','기존 고객·제품 정보를 조회해 반복 입력을 줄였습니다.')}<p class="us-side-copy">두 가지 실제 조회 흐름을 기준으로 기존 데이터를 재사용하도록 구성했습니다.</p><div class="us-reuse-grid">${reuse.map((x,ri)=>`<article><span class="us-card-icon">${kmrIcon(x[0])}</span><h3>${x[1]}</h3><div class="us-mini-flow">${x.slice(2).map((y,i)=>`${i?'<i>→</i>':''}<span>${y}</span>`).join('')}</div></article>`).join('')}</div><p class="us-reuse-takeaway">이미 존재하는 정보를 다시 입력하지 않고 조회·재사용하는 구조로 설계했습니다.</p></section>
    ...
    <section class="us-section us-outcome">${cap('08','Outcome','어떤 결과를 만들었나요?')}<div class="us-outcome-grid"><article class="us-highlight"><small>핵심 결과</small><h3>개별로 관리되던 고객·제품·서비스 기록을 <em>제조번호 중심의 CRM 이력</em>으로 연결했습니다.</h3><p>제조번호를 기준으로 고객·제품·설치·서비스 정보를 이어서 확인하고, 기존 정보를 재사용할 수 있는 구조를 구축했습니다.</p></article><div class="us-result-list">${results.map(x=>`<article><span>✓</span><div><h3>${x[0]}</h3><p>${x[1]}</p></div></article>`).join('')}</div></div></section>
...
function applyArchiveYears(){document.querySelectorAll('.project-card h3').forEach(h=>{h.textContent=h.textContent.replace(/^\d{4}\s*·\s*/, '')})}
window.addEventListener('hashchange',()=>setTimeout(applyArchiveYears,0));setTimeout(applyArchiveYears,0);
function applyCjCopy(){if(location.hash!=='#project/cj')return;const sections=[...document.querySelectorAll('.cj-section')];const setHeading=(n,text)=>{const h=sections[n]?.querySelector('.cj-cap h2');if(h)h.textContent=text};setHeading(3,'기존 기능과 추가 개발 범위를 구분했습니다.');setHeading(4,'주문 전체 정보와 배송지별 처리 단위를 분리했습니다.');setHeading(5,'출고 단위와 같은 기준으로 반품 대상을 연결했습니다.');const contexts=document.querySelectorAll('.cj-context');if(contexts[0])contexts[0].querySelector('p').textContent='퍼스트몰 기반 회원 전용 커머스를 구축하며, 사용자 프론트와 대량·다중배송·운영 백오피스까지 함께 설계했습니다.';if(contexts[1])contexts[1].querySelector('p').textContent='기존 솔루션 기능을 최대한 활용하면서, 필요한 추가 기능과 운영 기준을 구분해 구현 범위를 정의했습니다.';const outcome=document.querySelector('.cj-highlight');if(outcome){outcome.querySelector('small').textContent='핵심 결과';outcome.querySelector('h3').innerHTML='대량·다중배송 주문을 <em>배송지별로 출고하고 반품</em>할 수 있는 구조로 구현해 서비스를 오픈했습니다.';outcome.querySelector('p').textContent='퍼스트몰 기반으로 프론트와 백오피스를 함께 설계하고, 사용자 주문부터 운영 처리까지 이어지는 커머스 구조를 구축했습니다.'}}
window.addEventListener('hashchange',()=>setTimeout(applyCjCopy,0));setTimeout(applyCjCopy,0);
function applyKrVisuals(){
  if(location.hash!=='#project/kr-crm')return;
  const section=title=>[...document.querySelectorAll('.kr-section')].find(el=>el.querySelector('h2')?.textContent.trim()===title);
  const flow=section('상담 접수부터 완료까지 어떻게 이어졌나요?');
  if(flow)flow.innerHTML=`<div class="kr-cap"><span>04</span><div><small>Service Flow</small><h2>상담 접수부터 완료까지 어떻게 이어졌나요?</h2></div></div><p class="kr-visual-lead">담당자가 바뀌어도 하나의 서비스 건 안에서 이력과 후속 처리가 이어지도록 설계했습니다.</p><div class="kr-flow-visual"><div class="kr-flow-stack"><article><b>▣</b><div><strong>고객 상담 접수</strong><span>고객 문의 등록</span></div></article><i>↓</i><article><b>▤</b><div><strong>담당자 배정</strong><span>담당자 설정 · 일정 안내</span></div></article><i>↓</i><article><b>◯</b><div><strong>약속 · 방문</strong><span>방문 일정 관리 · 방문 결과 등록</span></div></article><i>↓</i><article class="is-branch"><b>⚒</b><div><strong>서비스 처리</strong><span>현장 서비스 진행 · 처리 내역 등록</span></div></article><i>↓</i><article><b>▦</b><div><strong>반품 · 결재</strong><span>반품 요청 · 승인 및 결재 처리</span></div></article></div><div class="kr-flow-extra"><span>추가 지원이 필요한 경우</span><div><article><b>◯</b><strong>기술지원 요청</strong><small>기술지원 요청</small></article><article><b>⌘</b><strong>컨설턴트 · 다른 담당자</strong><small>기술지원 진행</small></article><article><b>▣</b><strong>기술지원 완료</strong><small>지원 결과 등록</small></article></div><em>원 서비스 건으로 복귀</em></div><aside>여러 담당자가 이어서 처리하는 국내 CRM의 업무 흐름을 한눈에 보여줍니다.</aside></div>`;
  const status=section('서비스와 기술지원 상태를 각각 구분했습니다.');
  if(status)status.innerHTML=`<div class="kr-cap"><span>05</span><div><small>Status</small><h2>서비스와 기술지원 상태를 각각 구분했습니다.</h2></div></div><div class="kr-status-visual"><article><h3><b>●</b> 서비스 상태 <small>SERVICE STATUS</small></h3><div class="kr-status-line">${['접수|상담 등록','배정|담당자 지정','방문 예정|방문 일정 확정','처리 중|서비스 진행','완료|처리 완료'].map(x=>{const [a,b]=x.split('|');return `<span><i>◉</i><strong>${a}</strong><small>${b}</small></span>`}).join('')}</div></article><article><h3><b>◐</b> 기술지원 상태 <small>TECH SUPPORT STATUS</small></h3><div class="kr-status-line">${['요청|기술지원 요청','담당자 확인|담당자 배정','처리 중|기술지원 진행','기술지원 완료|처리 완료'].map(x=>{const [a,b]=x.split('|');return `<span><i>◉</i><strong>${a}</strong><small>${b}</small></span>`}).join('')}</div></article><aside><b>!</b><div><strong>기술지원 완료 ≠ 서비스 전체 완료</strong><p>기술지원 업무가 완료되어도, 서비스 건은 별도의 완료 기준에 따라 계속 진행될 수 있습니다.</p></div></aside></div>`;
  const permission=section('역할에 따라 조회 범위와 처리 권한을 구분했습니다.');
  if(permission)permission.innerHTML=`<div class="kr-cap"><span>07</span><div><small>Role / Permission</small><h2>역할에 따라 조회 범위와 처리 권한을 구분했습니다.</h2></div></div><div class="kr-permission-visual"><div class="kr-permission-table-visual"><div class="head"><b>역할</b><b>서비스 건 조회</b><b>처리 / 완료</b><b>기술지원 요청</b><b>이관 처리</b><b>통계 / 리포트</b><b>권한 관리</b></div>${[['기사','●','●','●','○','○','○'],['컨설턴트','●','○','●','●','○','○'],['CS · 관리자','●','●','●','●','●','●'],['기업영업 · 제품팀','●','○','○','○','●','○']].map(row=>`<div>${row.map((v,i)=>i?`<span class="${v==='●'?'on':''}">${v}</span>`:`<b>${v}</b>`).join('')}</div>`).join('')}</div><p><span>●</span> 권한 있음 <span>○</span> 권한 없음</p></div>`;
  const dashboard=section('역할마다 먼저 확인해야 할 업무가 달랐습니다.');
  if(dashboard)dashboard.innerHTML=`<div class="kr-cap"><span>09</span><div><small>Dashboard</small><h2>역할마다 먼저 확인해야 할 업무가 달랐습니다.</h2></div></div><div class="kr-dashboard-visual"><header><b>역할별 주요 업무</b><span class="active">기사</span><span>CS · 관리자</span><span>컨설턴트</span><span>결재자</span></header><div><article><h3>오늘의 업무</h3><p>▣ 방문 예정 <b>3건</b></p><p>⌘ 진행 중 <b>5건</b></p><p>⚒ 기술지원 요청 <b>1건</b></p><p>▦ 완료 대기 <b>2건</b></p></article><article><h3>최근 알림</h3><p><i>10:24</i> 기술지원 요청이 등록되었습니다.</p><p><i>09:12</i> 새로운 서비스 건이 배정되었습니다.</p><p><i>08:50</i> 방문 일정이 변경되었습니다.</p><p><i>08:10</i> 고객이 추가 요청하셨습니다.</p></article><article><h3>바로가기</h3><p>▣ 내 배정 건 보기</p><p>▤ 방문 일정 관리</p><p>⚒ 기술지원 요청하기</p><p>▦ 서비스 처리 등록</p></article></div></div>`;
}
window.addEventListener('hashchange',()=>setTimeout(applyKrVisuals,0));setTimeout(applyKrVisuals,0);
function applyO2Visuals(){
  if(location.hash!=='#project/o2o')return;
  const section=title=>[...document.querySelectorAll('.o2-section')].find(el=>el.querySelector('h2')?.textContent.trim()===title);
  const flow=section('온라인 주문부터 매장 픽업까지 어떻게 이어졌나요?');
  if(flow)flow.innerHTML=`<div class="o2-cap"><span>04</span><div><small>Case / Service Blueprint</small><h2>온라인 주문부터 매장 픽업까지 어떻게 이어졌나요?</h2></div></div><p class="o2-visual-lead">고객·쇼핑몰·매장이 하나의 주문 상태를 기준으로 처리하며, 주문부터 수령 완료까지 끊기지 않는 픽업 흐름을 설계했습니다.</p><div class="o2-blueprint"><div class="o2-blueprint-head"><b>주문</b><b>픽업 준비</b><b>준비 및 방문</b><b>수령 완료</b></div>${[['고객','상품 선택 · 픽업 매장 지정','주문 확인','방문 전 안내','매장 방문','상품 수령'],['쇼핑몰','픽업 주문 생성','주문 상태 관리','준비 완료 알림','–','픽업 완료 반영'],['매장','주문 확인','상품 준비','준비 완료 처리','바코드 확인','픽업 완료 처리']].map(row=>`<div class="o2-blueprint-row"><strong>${row[0]}</strong>${row.slice(1).map((item,i)=>`<span class="${(i===2&&row[0]!=='고객')?'focus':''}">${item}</span>`).join('')}</div>`).join('')}<aside>준비 완료 이후<br><b>수령 Barcode 노출</b></aside></div>`;
  const policy=section('상태에 따라 고객 정보와 바코드 노출 기준을 달리했습니다.');
  if(policy)policy.innerHTML=`<div class="o2-cap"><span>05</span><div><small>Policy / Data Rule</small><h2>상태별 고객 정보와 바코드 노출 기준</h2></div></div><p class="o2-visual-lead">시리얼 번호를 조회하고 재사용할 수 있도록 기존 데이터를 재활용하여 중복 입력을 줄이고, 업무 효율을 높였습니다.</p><div class="o2-policy-matrix"><div class="head"><b>주문 상태</b><b>주문 처리 정보</b><b>Barcode</b><b>운영 기준</b></div>${[['주문 완료','주문 내역 · 픽업 매장 · 예상 준비 시간','미노출','주문 접수 완료 상태'],['준비 완료 · 픽업 대기','주문 내역 · 상품 정보 · Barcode','노출','실제 수령 가능한 상태 (담당 확인 전)'],['픽업 완료','상품 수령 완료 · 처리 이력','미노출','고객 완료 처리 후 Barcode 비노출']].map((r,i)=>`<div class="${i===1?'active':''}">${r.map(x=>`<span>${x}</span>`).join('')}</div>`).join('')}<aside><b>ⓘ</b> Barcode는 주문 시점이 아니라 실제 수령 가능한 상태를 기준으로 노출합니다.</aside></div>`;
}
window.addEventListener('hashchange',()=>setTimeout(applyO2Visuals,0));setTimeout(applyO2Visuals,0);
function applyAuditContentFixes(){
  if(location.hash==='#project/o2o'){
    const policy=[...document.querySelectorAll('.o2-section')].find(el=>el.querySelector('h2')?.textContent.trim()==='상태별 고객 정보와 바코드 노출 기준');
    const lead=policy?.querySelector('.o2-visual-lead');
    if(lead)lead.textContent='주문 상태에 맞춰 고객에게 보여줄 정보와 매장 처리 기준을 정리하고, 실제 수령 가능한 시점에만 Barcode가 노출되도록 설계했습니다.';
  }
}
